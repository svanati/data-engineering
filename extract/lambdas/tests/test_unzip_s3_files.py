...
from unzip_s3_files import lambda_handler
...
def setup_mock_s3(mock_boto_client, zip_content=b""):
    ...
    Args:
        ...
        zip_content (bytes): Content of the zip src to be returned by the mock S3 client.
    ...
    event = {
        "Records": [
            {"s3": {"bucket": {"name": "test-bucket"}, "object": {"key": "test.zip"}}}
        ]
    }
...
def test_lambda_handler_successfully_unzips_multiple_files(mock_boto_client):
    ...
    mock_s3.put_object.assert_any_call(
        Bucket="test-bucket", Key="file1.txt", Body=b"content of file1"
    )
    mock_s3.put_object.assert_any_call(
        Bucket="test-bucket", Key="file2.txt", Body=b"content of file2"
    )
    mock_s3.put_object.assert_any_call(
        Bucket="test-bucket", Key="file3.txt", Body=b"content of file3"
    )
...
def test_lambda_handler_handles_empty_zip(mock_boto_client):
    r"""
    Test the lambda_handler function handles an empty zip src correctly.

    Args:
        mock_boto_client (MagicMock): Mocked boto3 client.
    """
    # Create an empty zip src in memory
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, "w"):
        pass  # Just create an empty zip src
    zip_buffer.seek(0)
...
def test_lambda_handler_handles_invalid_zip(mock_boto_client):
    r"""
    Test the lambda_handler function raises a BadZipFile exception for an invalid zip src.

    Args:
        mock_boto_client (MagicMock): Mocked boto3 client.
    """
    mock_s3, event = setup_mock_s3(mock_boto_client, zip_content=b"not a zip src")

    response = lambda_handler(event)

    assert response["statusCode"] == 400
    assert "Invalid zip file: File is not a zip file" in response["body"]
